...
# Ceph: what broke and got fixed

The Ceph field kit elsewhere on this site is what the environment's notes distilled. This entry is the repairs. One cluster, 2019 through 2023 — every fix below has a timestamp or a command output, because fixes without evidence are just stories. Hostnames genericized; the damage and the repair are as they happened.
...
The luminous→mimic run is [its own entry](../ceph-luminous-to-mimic/index.md); the failure it left — fifty-six seconds from `insufficient standby` to `MDS_ALL_DOWN` and back — is told there. The lesson in one line: `MDS_ALL_DOWN` is not a state you negotiate with; the standby was taking over before the health check finished changing its mind.
...
The newest incident, and the one that vindicates the split theory in [the EC retrospective](../ceph-ec-data-loss/index.md) — a loop detected on a switch, a port err-disabled, and eighty minutes later:
...
*Every fix above has a receipt. If your cluster starts writing its own — [info@wirt.ee](mailto:info@wirt.ee).*
